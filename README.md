...
**The stalls place themselves.** They are anchored by the **pixel positions of the fifty numbered red markers**, which is what keeps them traceable to the drawing — but unlike the hand-sketch version those anchors carry no direction. Fifty-one hand-written facings is fifty-one chances to be quietly wrong, and a wrong one used to make a stall vanish without a word. Instead each anchor is snapped to the nearest street cell (markers on the drawing sit on the buildings as often as on the road), and the nearest wall from there is the one it backs onto. Markers whose three-cell frontages would collide are dropped rather than stacked, since two boxes in one place read as one broken one: 43 of the 50 markers become stalls, and `probe_map` fails if fewer than 70% of them do — a bug in the snapping would drop most of them silently otherwise.
