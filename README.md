...
The visible body is `assets/enemies/hamburger_monster/hamburger_monster.glb`, scaled and grounded against the 4.10 m gameplay height. Its `Idle`, `Walk`, and biting `Attack` clips are selected by the existing chase and contact-hit states. Death is the procedural `fall_angle` motion described below: the imported Death clip is deliberately not played because it moves the rig a second time inside the already-fallen collision body. `BodyContamination` remains the deterministic record used by damage and network splat replay, its welded mesh hidden behind the authored visual, so replacing presentation did not change hit accounting, packet contents, or saved contamination state.
...
The imported `Walk` clip swings the arms from the shoulder and holds the body level, which reads as a burger hanging in the air being rowed along. `scripts/enemy_gait.gd` replaces that with contact: each palm gets a plant point, stays fixed while it carries the weight, then lifts and swings ahead. The arm is solved back from that point with two-bone IK, so the elbow bends because it is loaded rather than because a decorative bend was added.

The rig is short-armed: at the authored body height it can just hang its hands down, but has no reach left to move them fore and aft. The failed version solved that by raising the plant target, which produced a mathematically fixed hand in mid-air — exactly the floating paddle motion it was supposed to remove. The current gait keeps the asset's authored wrist height, lowers the body only by the compact amount allowed by `support_crouch_limit_metres`, and preserves the hand bone's actual rest-space contact basis so the palm—not the fist or hand edge—lies flat. A restrained stretch remains only as a safety limit; if a tuned stride still cannot fit within `arm_stretch_limit`, it is shortened instead of letting IK clamp the target.

That support crouch is followed by a small two-beat vertical bob, side-to-side weight roll, and restrained fore/aft pitch. Since gait phase comes from distance actually travelled rather than a clock, the body visibly passes over a planted palm and a slowed or shoved monster takes correspondingly shorter/slower steps instead of skating.

`probe_enemy` now checks the missing physical condition as well as the old timing conditions: the plant target must remain at the authored floor height, the required crouch must stay small, the planted hand must move far less than the swinging hand, and ground distance must map directly to gait phase.

### The stain is drawn from the rest pose

The overlay reads its mask by where a vertex sits in the body. That was the **animated** position, which meant every clip dragged the stain across the model. Worse, the imported Death clip moves the Body bone and both arms after `fall_angle` has already rotated the whole enemy: the visible corpse and its colliders separated by metres, so sauce fired after it fell was recorded on the collider and drawn somewhere else.
...
There is now one fall, not two. Death stops the AnimationPlayer, resets the skeleton to the same rest pose used by the fixed colliders and contamination projection, and lets the networked `fall_angle` rotate the node. `probe_enemy` kills a monster of its own, advances the complete procedural fall, and compares every visible mesh's actual body-space transform with the `rest_to_body` transform used by its overlay; a centimetre of separation fails the test. That catches both an old stain sliding and a new stain appearing away from the point that was hit.
