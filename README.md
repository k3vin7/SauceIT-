...
**The stalls place themselves.** Every one of the 51 numbered markers on the drawing is a food or drink vendor, so every one of them gets a stall, anchored by its pixel position — but unlike the hand-sketch version the anchors carry no direction. Fifty-one hand-written facings is fifty-one chances to be quietly wrong, and a wrong one used to make a stall vanish without a word. Instead each anchor is snapped to the nearest street cell (markers on the drawing sit on the buildings as often as on the road), and the nearest wall from there is the one it backs onto. Where two would stand on the same ground — the markers cluster more tightly than a frontage allows — the second slides along the wall to the nearest free spot rather than being dropped, **re-finding the wall as it goes**: around the round place the rim curves away after a couple of cells, and insisting on the wall the search started against left the last marker on the map with nowhere to stand. `probe_map` fails unless all 51 place and none share a cell.

**A stall is a 3 m × 3 m pop-up gazebo**, which is what a market pitch actually is — four legs, a canopy at about 2.2 m, a peak at 3.27 m, and a serving counter at waist height. Those are real metres and this world is not built in them: its people are the prototype's 2.56 m capsule rather than a 1.75 m human, so a literal 3.27 m canopy would clear a player's head by 0.7 m and read as a toy. The real figures are kept as the real figures and scaled once by the ratio between the two, which puts the pitch at 4.39 m square and the peak at 4.78 m.

Two parts of it take sauce and are registered as walls: the **counter**, which is at the height things get sprayed at and the part you duck behind, and the **canopy**, which catches anything fired over it. The legs get collision but no grid — four more masks a few cells across, for a stain nobody can see. The counter being waist-high is the point of the change: the solid block that stood here while the street was being laid out was cover you could hide behind completely, and a counter is cover you shoot *over*, which is the more interesting half of what a market stall is for.
