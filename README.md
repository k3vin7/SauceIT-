...
3. Move with `WASD`, aim with the mouse cursor, and hold the left mouse button to fire. `Esc` exits.
...
This step intentionally contains only WASD movement, mouse aiming, one mayonnaise strand, static wall collision, flat-floor and wall contamination, ribbon shadow, and the seven-droplet landing accent. There is no UI, sound, inventory, recharge, extra sauce, enemy, or networking code.
