...
`scripts/enemy.gd` keeps the original capsule figure — head, torso, hips, two arms, two legs — as its gameplay body and deterministic sauce-mask target, but no longer renders that mockup. The visible body is `assets/enemies/hamburger_monster/hamburger_monster.glb`, scaled and grounded against the same 4.10 m gameplay height. Its `Idle`, `Walk`, biting `Attack`, and `Death` clips are selected by the existing chase, contact-hit, and death states. Health, movement speed, target selection, contact reach and cooldown, colliders, spawn positions, network packet shape, and health-bar/minimap hooks remain on `MayoEnemy` unchanged.

**The unwrap still uses the torso's radius rather than the figure's.** `BodyContamination` remains the deterministic record used by damage and network splat replay, even though its legacy welded mesh is now hidden behind the authored visual. It was deliberately retained so replacing presentation did not change hit accounting, packet contents, or saved contamination state.
